...
telegram()
{
	...
	echo "# Following coins are SOMed as newly listed - less than $tPday days old" >> $file
        for i in $tgOut
	do
		exchangeFormat $exchange $i $tMarket
		echo  $cEx"_sell_only_mode = true" >> $file
...
if [ "$tg" != "0" ]; then
	...
	telegram $tg $market $exchange $file

fi

echo "#Coin specific SOM - limit +- $limit%" >> $file
for i in $list 
do
